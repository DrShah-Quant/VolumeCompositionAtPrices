...
if __name__ == '__main__':
            ...
            for row in rows:  
                if(row["Type"] == 'B'):
                    if(buyUniquePriceSet.isdisjoint({float(row["Price"])})):
                        ...
                        buyPriceVolumeDictionary[float(row["Price"])] = float(row["Vol"])
                    ...
                    if(sellUniquePriceSet.isdisjoint({float(row["Price"])})):
                        ...
                        sellPriceVolumeDictionary[float(row["Price"])] = float(row["Vol"])
                    ...
                    if(openUniquePriceSet.isdisjoint({float(row["Price"])})):
                        ...
                        openPriceVolumeDictionary[float(row["Price"])] = float(row["Vol"])
    ...
    for sDate in sellDataDictionary.keys():
        ...
        for sPrice in sellDataDictionary[sDate]:
            ...
            if sDate in buyDataDictionary:
                if sPrice in buyDataDictionary[sDate]:
                    volumePosSell.append(float(buyDataDictionary[sDate][sPrice]))
                else:
    ...
    for oDate in openDataDictionary.keys():
        for oPrice in openDataDictionary[oDate]:
            ...
            if oDate in buyDataDictionary:
                if oPrice in buyDataDictionary[oDate]:
                    if oDate in sellDataDictionary:
                        if oPrice in sellDataDictionary[oDate]:
                            volumePosOpen.append(float(buyDataDictionary[oDate][oPrice]) + float(sellDataDictionary[oDate][oPrice]) )
                        else: 
                            volumePosOpen.append(float(buyDataDictionary[oDate][oPrice]))
                    else:
                        volumePosOpen.append(float(buyDataDictionary[oDate][oPrice]))
                else:
